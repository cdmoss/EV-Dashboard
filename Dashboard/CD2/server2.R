...
shinyServer(function(input, output, session) {
  ...
  df <- reactive({
    ...
    #tyy <<- dat
    #the indexes being called depend on the inputs from the ui
  ...
  limiter = function(dataframe, x = "x_var", y = "y_var", cor_lim = 0.7, rsq_lim =  0.5){
  ...
  output$mainChart <- renderPlotly({
    ...
    p = p +
      ...
      scale_fille_manual(values = c("#2196f3", "#771a39", "#358a93", "#1b1e4", "#bfaac1"))
    
    ggplot_plot <<- p
    
    plotly_plot <<- p %>% 
      ggplotly(ttoltip = "text") %>% 
