...
Range is significantly affected by temperature. 

```{r echo = FALSE, message=FALSE, warning=FALSE}
df %>% 
  group_by(`Temperature(C)` = cut(start_outside_temp, 5)) %>% 
  summarise(`Max Range` = max(range_start, na.rm = T) %>% round(0)) %>% 
  drop_na %>%
  mutate(`Temperature(C)` = `Temperature(C)` %>% 
           str_remove("\\(") %>% 
           str_remove("\\]") %>% 
           str_replace("\\,", " to ")) %>% kable
```

##### How does temperature affect the range of the vehicle?
...
<img src="EV_439.jpg" style="width:700px;">
